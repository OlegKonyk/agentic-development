...
- The contract tier is deterministic, and that determinism is why a failure is worth your time. It replays a committed corpus (`qa/tests/contract/corpus.json`) instead of generating on the fly, so there is no PRNG or import-order sensitivity in the gating path — the verdict is a pure function of the commit, unlike the PR #14 flip (Hypothesis's local-constants pool varying across processes) that motivated this design. A contract failure is a real finding to triage, not a coin flip; a drift failure (the committed corpus disagreeing with the live schema) means refresh it with `make contract-refresh` in the same PR. Anything the tier catches once still gets pinned as a deterministic test — the corpus alone is not enough, since it can be regenerated away (precedent: the NUL/surrogate cases in `apps/api/tests/test_auth.py` and `test_tasks.py`).
